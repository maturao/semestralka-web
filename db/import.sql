...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `maturao_article_view` (`id` INT, `id_user` INT, `display_name` INT, `abstract` INT, `pdf_file` INT, `id_article_state` INT, `user` INT, `article_state` INT);

-- -----------------------------------------------------
-- Placeholder table for view `maturao_user_view`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `maturao_user_view` (`id` INT, `login` INT, `password` INT, `id_role` INT, `role` INT);

-- -----------------------------------------------------
-- Placeholder table for view `maturao_review_view`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `maturao_review_view` (`id` INT, `id_user` INT, `id_article` INT, `content_quality` INT, `technical_quality` INT, `language_quality` INT, `user` INT, `article` INT);
...
-- -----------------------------------------------------
-- View `maturao_user_view`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `maturao_user_view`;
DROP VIEW IF EXISTS `maturao_user_view` ;
CREATE  OR REPLACE VIEW `maturao_user_view` AS
    SELECT 
        `maturao_user`.*, 
        `maturao_role`.display_name as `role`
    FROM
        `maturao_user`
            INNER JOIN
        `maturao_role` ON `maturao_user`.id_role = `maturao_role`.id;

-- -----------------------------------------------------
-- View `maturao_review_view`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `maturao_review_view`;
DROP VIEW IF EXISTS `maturao_review_view` ;
CREATE  OR REPLACE VIEW `maturao_review_view` AS
    SELECT 
        `maturao_review`.*,
        `maturao_user`.login AS `user`,
        `maturao_article`.display_name AS `article`
    FROM
        `maturao_review`
            INNER JOIN
        `maturao_user` ON `maturao_review`.id_user = `maturao_user`.id
            INNER JOIN
        `maturao_article` ON `maturao_review`.id_article = `maturao_article`.id;
